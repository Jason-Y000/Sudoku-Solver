...
model_path = # Path to the file with the pre-trained model's weights
...
while True:
    ...
    if time_elapsed > 1./frame_rate:
        ...
        if (np.array_equal(frame_temp,frame)):
            ...
            # grid = predictDigits(model,model_path,cells)
            ...
            # if not all_board_non_zero(solution.solved):
            if not solved:
                frame_temp = cv2.resize(frame_temp,(1066,600))
                cv2.imshow('frame',frame_temp)
                # cv2.destroyAllWindows('solved')
            ...
            # annotated = printDigits(frame,im_adjusted,grid,transformed_pts,rect)
