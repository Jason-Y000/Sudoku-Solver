...
def preprocess_image(path): # Returns a list of the individual cells of the sudoku board
  ...
  # Transform perspective of the original image into a top-down view
  im1 = cv2.imread(path)
  ...
  cells = []
  for row in range(0,im_adjusted.shape[0],cell_h):
    temp = im_adjusted[row:row+cell_h+1]
    temps = []
    for col in range(0,im_adjusted.shape[1],cell_w):
      tempgrid = []
      for r in temp:
